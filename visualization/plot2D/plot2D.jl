...
function printProfiles_IP(case, t)
    ...
    pos_Q_index = argmax(zQ)
    pos_T_index = argmax(zTm)

    xlims = (0, Sx)
    ylims = (0, Sy)

    plt_T_Profile = heatmap(X, Y, zT, xlims=xlims, ylims=ylims, clims=(T_min, T_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Temperature", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature $[\mathrm{K}]$", dpi=1000)
    savefig(plt_T_Profile, joinpath(imagePath, case, "TemperatureProfile_" * string(t) * ".pdf"))

    plt_cT_Profile = heatmap(X, Y, zcT, xlims=xlims, ylims=ylims, yflip=false, c=colgrad, aspect_ratio=:equal, title="Temperature Standard Deviation", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature Standard Deviation $[\mathrm{K}]$", dpi=1000)
    savefig(plt_cT_Profile, joinpath(imagePath, case, "TemperatureCovarianceProfile_" * string(t) * ".pdf"))

    plt_Q_Profile = heatmap(X, Y, zQ, xlims=xlims, ylims=ylims, clims=(Q_min, Q_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Heat Flux", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Heat Flux $[\mathrm{W/m}^2]$", dpi=1000)
    savefig(plt_Q_Profile, joinpath(imagePath, case, "HeatFluxProfile_" * string(t) * ".pdf"))

    plt_cQ_Profile = heatmap(X, Y, zcQ, xlims=xlims, ylims=ylims, yflip=false, c=colgrad, aspect_ratio=:equal, title="Heat Flux Standard Deviation", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Heat Flux Standard Deviation $[\mathrm{W/m}^2]$", dpi=1000)
    savefig(plt_cQ_Profile, joinpath(imagePath, case, "HeatFluxCovarianceProfile_" * string(t) * ".pdf"))

    plt_Tm_Profile = heatmap(X, Y, zTm, xlims=xlims, ylims=ylims, clims=(Tm_min, Tm_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Observed Temperature", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature $[\mathrm{K}]$", dpi=1000)
    savefig(plt_Tm_Profile, joinpath(imagePath, case, "ObservedTemperatureProfile_" * string(t) * ".pdf"))

    plt_cTm_Profile = heatmap(X, Y, zcTm, xlims=xlims, ylims=ylims, yflip=false, c=colgrad, aspect_ratio=:equal, title="Observed Temperature Deviation", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature Standard Deviation $[\mathrm{K}]$", dpi=1000)
    savefig(plt_cTm_Profile, joinpath(imagePath, case, "ObservedTemperatureCovarianceProfile_" * string(t) * ".pdf"))

    plt_TsN_Profile = heatmap(X, Y, zTsN, xlims=xlims, ylims=ylims, clims=(Tm_min, Tm_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Measured Temperature", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature $[\mathrm{K}]$", dpi=1000)
    savefig(plt_TsN_Profile, joinpath(imagePath, case, "NoisyMeasureTemperatureProfile_" * string(t) * ".pdf"))

    plt_Ts_Profile = heatmap(X, Y, zTs, xlims=xlims, ylims=ylims, clims=(Tm_min, Tm_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Measured Temperature", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Temperature $[\mathrm{K}]$", dpi=1000)
    savefig(plt_Ts_Profile, joinpath(imagePath, case, "MeasureTemperatureProfile_" * string(t) * ".pdf"))

    plt_Qs_Profile = heatmap(X, Y, zQs, xlims=xlims, ylims=ylims, clims=(Q_min, Q_max), yflip=false, c=colgrad, aspect_ratio=:equal, title="Reference Heat Flux", xlabel=L"X $[\mathrm{m}]$", ylabel=L"Y $[\mathrm{m}]$", colorbar_title=L"Heat Flux $[\mathrm{W/m}^2]$", dpi=1000)
...
function printEvolutions(case, x, y, isExperimental)
    ...
    plt_Ts_Evolution = plot(T, zTs, title="Temperature", xlabel=L"Time $[\mathrm{s}]$", ylabel=L"Temperature $[\mathrm{K}]$", label=labels[:T], ls=linestyles[:T], lw=linewidths[:T], seriescolor=seriescolors[:T], dpi=1000)
    savefig(plt_Ts_Evolution, joinpath(imagePath, case, "MeasuredTemperatureEvolution_" * string(x) * "_" * string(y) * ".pdf"))
